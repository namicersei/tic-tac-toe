...
let BoardComponent = class BoardComponent {
    constructor(winnercheckService) {
...
let WinnercheckService = class WinnercheckService {
    constructor(winnerCheckService) {
        this.winnerCheckService = winnerCheckService;
        this.dimension = 3;
        this.board = Array(9).fill(null);
    ...
    checkMainDiag(twoDimensionalBoard, i, j, mark, parentI, parentJ) {
    ...
    checkSideDiag(twoDimensionalBoard, i, j, mark, parentI, parentJ) {
        if (i >= this.dimension || i < 0 || j < 0 || j >= this.dimension) {
            return 0;
        }
        if (twoDimensionalBoard[i][j] !== mark) {
    ...
    checkColumn(twoDimensionalBoard, i, j, mark, parentI, parentJ) {
    ...
    checkRow(twoDimensionalBoard, i, j, mark, parentI, parentJ) {
        if (this.count === this.dimension) {
            return [];
        }
        if (i >= this.dimension || i < 0 || j < 0 || j >= this.dimension) {
    ...
    checkWinner(board, startingPoint) {
        this.count = 0;
        let twoDimensionalBoard;
        twoDimensionalBoard = [];
...
WinnercheckService.ctorParameters = () => [
